...
export class Transliterator {
  ...
  constructor() {
    for (const [source, replacement] of Object.entries(DEFAULT_APPROXIMATIONS)) {
      this.approximate(source, replacement);
    }
  }
...
export function transliterations(locale = "en") {
  instances[locale] ??= new Transliterator();
  return instances[locale];
}

export function setTransliterations(locale: string, fn: (transliterator: Transliterator) => void) {
  fn(transliterations(locale));
}

export function transliterate(string: string, options: { locale?: string; replacement?: string } = {}) {
